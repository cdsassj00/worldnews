...
**2026-09-10: `horizons`(투자 기간별 추천) 추가 — §14. 발행 채널은 이제 이 블록을 우선 읽는다.**
...
## 14) `horizons` — 투자 기간별 종목 추천 (2026-09-10 추가) ★

**이 절이 지금 가장 중요한 블록이다.** 이전까지 발행하던 `picks`·`swing` 은 기간 구분이 없어,
받는 사람이 하루 만에 팔지 석 달을 들지 알 수 없었다. `horizons` 는 그 답을 담는다.

### 14-1) 구조

`briefs[].horizons` (없으면 `null`):

| 필드 | 뜻 |
|---|---|
| `entryKo` | 매수 시점 — 세 구간 공통 |
| `noteKo` | 세 구간의 관계 한 문장 |
| `buckets[]` | 단타 / 스윙 / 장기 |
| `caveats[]` | 이 규칙들을 그대로 믿으면 안 되는 이유 |
| `measuredAt` | 백테스트 측정일 |
| `disclaimerKo` | 면책 |
...
| `id` | `"day"` / `"swing"` / `"long"` |
| `nameKo` | 단타 / 스윙 / 장기 |
| `holdKo` | **실측** 평균 보유 기간 (예측이 아님) |
| `ruleKo` | 청산 규칙 한 문장 |
| `entryKo` | 매수 시점 |
| `orderKo` | 정렬 기준 **과 그 한계** |
| `track` | 그 규칙의 백테스트 성적 (`returns`·`winRate`·`trades`·`maxDd`·`holdDaysAvg`·`benchmarkReturns`·`summaryKo`) |
| `picks[]` | 종목 |
| `cautionKo` | 이 구간을 쓸 때 알아야 할 것 |
...
|---|---|
| `combo` | 삼합 총점과 축별 점수 |
| `why.ontologyKo` | **거시 관점** — 어떤 거시 요인이 이 업종을 미는가 |
| `why.flowKo` | **수급 관점** — 거래대금·자금흐름·매집 |
| `why.chartKo` | **차트 관점** — 전략 13종 합의 + 이평 대비 위치 |
| `plan.stop` / `stopPct` | 손절 절대가·비율 |
| `plan.target` / `targetPct` | 목표 절대가·비율 (장기는 `null`) |
| `plan.rr` | 손익비 (목표 없으면 `null`) |
| `plan.nearestSupport` / `nearestResistance` | **참고** 지지·저항 (계획을 바꾸지 않음) |
| `plan.levelNoteKo` | 규칙 손절·목표와 실제 지지·저항의 관계 한 문장 |
| `views.chart` / `strategies` / `stock` | 그대로 첨부할 장면 주소 |

### 14-2) 세 구간의 규칙과 성적 (2026-09-10 측정)

**매수 신호는 셋 다 같다** — 삼합(온톨로지+수급+차트 평균) 점수 0.35 문턱.
다른 것은 언제 파느냐뿐이고, 규칙마다 백테스트를 따로 돌렸다.

| 구간 | 청산 규칙 | 한국 1년 | 미국 1년 | 실측 보유 |
|---|---|---|---|---|
| 단타 | 익절 +10% · 손절 −5% · 7일 시간청산 | +7.6% (54건, 35%) | +26.3% (164건, 40%) | 1~3일 / 4~8일 |
| 스윙 | 익절 +15% · 손절 −6% · 섹터당 2종목 | +53.8% (138건, 36%) | +24.2% (96건, 34%) | 3~4일 / 12~16일 |
| 장기 | 익절 없음 · 고점 대비 −25% 추적손절 | +151.6% (37건, 54%) | +122.8% (10건, 70%) | 29~41일 / 60~170일 |

같은 기간 벤치마크: 코스피 +112.2%, S&P500 +16.4%.
**한국 단타·스윙은 코스피 매수 후 보유에 크게 진다.** 콘텐츠에 쓸 때 이 사실을 빼면 안 된다.

### 14-3) 발행할 때 반드시 지킬 것

1. **매수 시점을 빼지 말 것.** "마감 후 점수 → 다음 거래일 시가"가 백테스트가 검증한 체결
   시점이다. 장중 점수로 장중에 사는 건 검증한 적 없다.
2. **`holdKo` 는 실측이지 약속이 아니다.** "3일 안에 오른다"로 바꿔 쓰면 거짓이 된다.
3. **`track` 을 같이 낼 것.** 수익률만 크게 쓰고 표본수·승률·낙폭을 빼면 광고가 된다.
   장기 미국은 1년에 10건뿐이다.
4. **`plan.stop`·`target` 은 측정한 규칙 그대로다.** 지지선으로 바꾸지 말 것 — 계획과
   성적표가 다른 규칙을 가리키면 성적표가 의미를 잃는다. 지지·저항은 `nearestSupport`·
   `levelNoteKo` 로 따로 실어 두었다.
5. **`orderKo` 를 읽을 것.** 단타·장기의 종목 정렬(수급 축·거시 축 우선)은 따로 측정하지
   않았다. 문턱과 청산 규칙만 측정된 값이다.

### 14-4) 장면(이미지)

- `GET /api/scene.svg?view=horizons&market=KR` — 세 구간을 한 장에 (1920×1080)
- 종목별은 `view=chart:<코드>` · `view=strategies:<코드>` · `view=stock:<코드>`
- 텔레그램은 이 넷을 앨범 한 묶음(`sendMediaGroup`)으로 보낸다 — 낱장 발송은 알림이 여러 번 울린다.

### 14-5) 텔레그램 명령

- `/추천` — 세 구간 전부 + `horizons`·`combo`·`flow` 이미지
- `/단타` `/스윙` `/장기` — 그 구간만 + 종목 일봉
